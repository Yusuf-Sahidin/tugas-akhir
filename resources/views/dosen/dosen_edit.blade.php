...
@section('container')
    <div class="container">
        <form action="{{ route('edit_dosen',  $dosen->user_id) }}" method="POST">
            @csrf
            @method('patch')
            ...
            <div class="form-group">
                ...
                <select name="nama_jurusan" id="nama_jurusan" class="form-control" style="width: 350px" required >
                    ...
                    @foreach ($data_jurusan as $item)
                        <option value="{{ $item->id }}" {{ $dosen->jurusan_id == $item->id ? 'selected' : null  }}>{{ $item->jenjang.' '.$item->nama_jurusan }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
